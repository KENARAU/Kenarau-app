...
Escribe SOLO el guión, sin títulos ni numeración. Usa emojis estratégicamente. Máximo 80 palabras. Tono cercano y latino.`;
  ...
  if (!response.ok) throw new Error("Error al conectar con Claude API");
    ...
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", background: C.bg }}>
      <div style={{ width: 360, padding: 32 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{ fontSize: 36, fontWeight: 800, color: C.purpleLight, marginBottom: 8 }}>KENARAU</div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 24, background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: 4 }}>
          {["login", "registro"].map(m => (
            <button key={m} onClick={() => { setModo(m); setError(""); setMensaje(""); }} style={{
              flex: 1, padding: "8px 0", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, fontFamily: "'DM Sans',sans-serif",
              background: modo === m ? gradPurple : "transparent",
              color: modo === m ? "white" : C.textSecondary,
            }}>{m === "login" ? "Ingresar" : "Crear cuenta"}</button>
          ))}
        </div>
        ...
        <button style={{ ...s.btnPrimary, opacity: cargando ? 0.7 : 1 }}
          onClick={modo === "login" ? handleLogin : handleRegistro} disabled={cargando}>
          {cargando ? "Cargando..." : modo === "login" ? "Ingresar →" : "Crear cuenta gratis →"}
        </button>
      </div>
    </div>
  );
    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 18, fontWeight: 700 }}>Hola 👋</div>
        ...
        <button onClick={onLogout} style={{ ...s.btnSecondary, fontSize: 12 }}>Cerrar sesión</button>
      </div>
...
function Avatar4({ setScreen }) {
  return (
    <div style={{ textAlign: "center", padding: "40px 20px" }}>
      ...
      <button style={s.btnPrimary} onClick={() => setScreen("mimodelo")}>Ver mi modelo ❤️</button>
    </div>
  );
}
...
function NuevoVideo({ setScreen, setGuionData }) {
  ...
  const handleGenerar = async () => {
    if (!desc.trim()) { setError("Por favor describe tu producto o servicio."); return; }
  ...
  return (
    <div>
      ...
      <div style={{ ...s.card, marginBottom: 16, background: "rgba(124,92,252,0.05)", borderColor: "rgba(124,92,252,0.2)" }}>
        <div style={{ fontSize: 12, color: C.purpleLight, marginBottom: 12 }}>✨ Generado por IA · Claude</div>
        {editando ? (
          <textarea style={{ ...s.input, height: 200, resize: "vertical", lineHeight: 1.8 }} value={texto} onChange={e => setTexto(e.target.value)} />
        ) : (
          <div style={{ fontSize: 14, lineHeight: 1.8, color: "rgba(255,255,255,0.85)", whiteSpace: "pre-wrap" }}>{texto}</div>
        )}
      </div>
...
export default function App() {
  ...
  if (cargandoAuth) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", background: C.bg, color: C.purpleLight, fontSize: 18, fontFamily: "'DM Sans',sans-serif" }}>
        Cargando...
      </div>
    );
  }
  ...
  const renderScreen = () => {
    switch (screen) {
      case "home": return <Home setScreen={setScreen} user={user} onLogout={handleLogout} />;
      ...
      default: return <Home setScreen={setScreen} user={user} onLogout={handleLogout} />;
    }
  };
    ...
    <div style={s.app}>
      <div style={s.sidebar}>
        ...
        <div style={{ marginTop: "auto", paddingTop: 12 }}>
          <button onClick={handleLogout} style={{ ...s.btnSecondary, width: "100%", fontSize: 11 }}>Cerrar sesión</button>
        </div>
      </div>
      <div style={s.main}>
        {renderScreen()}
